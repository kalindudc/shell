...
# Only run compinit with -C (skip security check) unless .zcompdump is older than .zshrc
...
# Initialize pyenv - must be eager to persist global version across sessions
...
# Ruby user gem bin - ensures `bundle`, `rubocop`, etc. are available without root
# and without needing rbenv to be fully initialised (e.g. inside task/make).
...
# Source .env if it exists and is not empty
if [[ -s $HOME/.env ]]; then
  set -a
  source $HOME/.env
  set +a
