...
Read PR description for intent. When the description is thorough with structured sections, leverage it to skip exploratory investigation and focus on validating claims against the diff. Read changed files in full context (not just diff hunks). Use `ast_query` for structural patterns if needed. Use `@codebase-explorer` if codebase is unfamiliar.
