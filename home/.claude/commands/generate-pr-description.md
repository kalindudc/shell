...
Automatically generates a PR description for the current branch by analysing git changes and following the required format standards: $ARGUMENT

## Auto-Approvals Configuration

This command runs smoothly without permission prompts because the following auto-approvals are configured globally in `~/.claude/settings.json`:

**Configured permissions:**
- `Bash(git:*)` - All git read commands (status, branch, diff, log)
- `Bash(which:*)` - Check for CLI tools (graphite)
- `Bash(gt:*)` - Graphite CLI commands
- `Bash(mkdir:*)` - Create directories in /tmp
- `Bash(cat:*)` - Read and write files using cat
- `Bash(cat /tmp:*)` - Read files from /tmp (including piped commands)
- `Bash(pbcopy:*)` - Copy to clipboard
- `Bash(rm:*)` - Remove files
- `Bash(touch:*)` - Create empty files
- `Read(/tmp/**)` - Read temporary analysis files
- `Read(~/.templates/**)` - Read PR template
- `Write(/tmp/**)` - Write analysis outputs

These permissions allow the command to run autonomously without interrupting your workflow.

## Pre-Requisites and Requirements
- ALWAYS use the `Write` tool instead of using `>` when updating a file or create and writing to a new file
...
1. First, determine the parent branch:
    ...
    - As a last resort use the `main` branch as the parent branch
...
4. Read the PR template from `$HOME/.templates/pr_description.md`
